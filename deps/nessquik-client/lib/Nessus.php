...
class Nessus {
	...
	* @var array 	
	*/
	private $host_full_issues	= array();
	...
	*/
	public function output_html($nbe = '', $embedded = false) {
		if ($nbe != '') {
			if (!is_array($nbe))
				$nbe = explode(':::', $nbe);
	...
	*/
	private function portname_to_ahref($name, $hostname) {
		$new_anchor = '';
	...
	*/
	private function set_named($name) {
	...
	*/
	private function has_name($name) {
	...
	*/
	private function correct_p_tags($content) {
	...
	*	the two timestamps.
	*/
	private function call_duration($dateTimeBegin,$dateTimeEnd) {
