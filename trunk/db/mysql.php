...
*
* Abstraction layer for accessing MySQL databases
*
* @author Tim Rupp
*/
class MySQL_DB {
	const _CONNECT_ERROR	= "Could not connect to the database";
	const _DB_SELECT_ERROR	= "Database does not exist!";
	...
	public function connect() {
		...
		*/
		if (_CONNECT_TYPE == "persist") {
			...
			*/
			@$this->link = mysql_pconnect(	$this->server, 
							$this->username, 
							$this->password);
		} else {
			/**
			* A normal connection. This results in each request by the user requiring a 
			* new connection to the database. The 'true' in this forces new links to be opened
			*/
		...
		*/
		if (!is_resource($this->link)) {
			...
			if (_DEBUG) {
				throw(new MySQL_Exception(self::_CONNECT_ERROR));
			} else {
		...
		*/
		if ($this->db != "") {
			if (!mysql_select_db($this->db, $this->link)) {
				$this->error = true;
				if (_DEBUG) {
					throw(new MySQL_Exception(self::_DB_SELECT_ERROR));
				} else {
					die(self::_DB_SELECT_ERROR);
				}
			}
		}
	}
	...
	*
	* @return MySQL_DB_Statement Object containing database connection and query to execute
	*/
	public function prepare($query) {
		...
		*/
		return new MySQL_DB_Statement($this->link, $query);
	}
	...
	*
	* @author phpMyAdmin maintainers
	* @author Tim Rupp Modified by me for inclusion in nessquik
...
class MySQL_DB_Statement {
	...
	*/
	public function __construct($link, $query) {
		...
		if (!is_resource($link)) {
			$this->error = true;
			if (_DEBUG) {
				throw(new MySQL_Exception(self::_INVALID_CONNECTION));
			} else {
	...
	*/
	public function fetch_row() {
		if(!$this->result) {
			$this->error = true;
			if (_DEBUG) {
				throw(new MySQL_Exception(self::_QUERY_NOT_EXEC));
			} else {
		...
		*/
		if (count($binds) > 0) {
			...
			foreach ($this->binds as $key => $val) {
				...
				* function call.
				*/
				$query = preg_replace("/:$key/", mysql_real_escape_string($val), $query, 1);		
			}
		} else {
			$query 	= $this->query;
		}
	...
	*/
	public function field($index = 0) {
		...
		return mysql_fetch_field($this->result, $index);
	}
	...
	*/
	...
	/**
	* Returns the last auot_increment ID of an INSERT
	*
...
* @author Tim Rupp
*/
class MySQL_General_Exception extends Exception {
	/**
	...
	*/
	...
	/**
	* Creates an instance of MySQL_General_Exception class
	*
	* This is a default constructor to override the one otherwise
...
* @author Tim Rupp
*/
class MySQL_Exception extends MySQL_General_Exception {
	/**
	...
	*/
	...
	/**
	* Creates an instance of MySQL_Exception class
	*
	* This is a default constructor to override the one otherwise
	* created by PHP. This constructor need not do anything complex
	* so a basic one is provided. This constructor begins by setting
